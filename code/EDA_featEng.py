...
print("--- Loading Data ---")
df = pd.read_csv('data/mod_data/receivals_clean_data.csv')
...
daily_net = (
    df_net.groupby(['rm_id', 'date_arrival'], as_index=False)['net_weight']
    .sum()
)
...
cum_out = final_df[['rm_id', 'date_arrival', 'cum_net_weight']]
...
# Should I to just train a model per rm_id or include rm_id as categorical feature in one model?
# We can try both approaches and compare performance.
# We can also try to take rm_ids that are always zero and set them manually to zero in predictions and
# exclude them from the model training.